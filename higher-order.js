...
//Code Here
let postTaxPrices = prices.map((price)=>price=price*1.07);
...
let ordersTotal = orders.map((x)=>x[0]+x[1])
...
const bobsTotal = purchases.filter((z)=>{return z['owner']=='Bob'}).reduce((x,y)=>{return x+y['price']},0);
